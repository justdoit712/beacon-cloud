...
6. `PUT /cache/strings/{key}`
- 请求体：`{"value":"...","ttlSeconds":600}`
- 返回：`ResultVO<Void>`
- 说明：覆盖写入字符串值；`ttlSeconds` 为空或小于等于 0 时表示不额外设置 TTL

7. `DELETE /cache/strings/{key}`
- 返回：`ResultVO<String>`
- 说明：原子读取并删除字符串值；适合一次性消费中间状态

8. `GET /v2/cache/set/{key}/string-members`
- 返回：`ResultVO<Set<String>>`

9. `GET /v2/cache/set/{key}/map-members`
