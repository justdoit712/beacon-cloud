...
## 7.4 扣费字段一致性风险已部分收敛（中风险）

文件：`FeeStrategyFilter`

1. 当前 `FeeStrategyFilter` 对 `fee/clientId` 空值与远程扣费异常增加了兜底，失败时会写日志、发失败回调并抛业务异常。
2. 但 `beacon-api` 的 internal 入口仍不走 `checkFilterContext`，`submit.fee` 仍可能缺失，导致请求在策略层被判失败。
3. 当前风险主要表现为“入口字段不一致导致的业务失败”。
...
1. 修复路由 `TreeSet` 去重问题，改为 List 排序并保留同权重元素。
2. 继续统一入口字段约束与扣费前置条件，避免 internal 入口缺 `fee` 时在策略层失败。
3. 给失败回调流程补齐空值防御（`isCallback`）。
...
`beacon-strategy` 是平台稳定性的核心模块，当前功能覆盖全面，且策略链具备动态配置能力。  
主要问题集中在“一致性与可运营性”：同类策略行为一致性、入口字段一致性、路由稳定性、关键参数配置化。  
